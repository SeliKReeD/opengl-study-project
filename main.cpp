...
int main()
{
    ...
    Shader paralaxShader("vertex_paralax.glsl", "fragment_paralax.glsl");
...
#pragma endregion
    ...
    unsigned int diffuseParalaxMap = loadTexture("./Images/bricks2.jpg");
    unsigned int normalParalaxMap = loadTexture("./Images/bricks2_normal.jpg");
    unsigned int heightParalaxMap = loadTexture("./Images/bricks2_disp.jpg");   

    // Shader configuration
    paralaxShader.use();
    paralaxShader.setInt("diffuseMap", 0);
    paralaxShader.setInt("normalMap", 1);
    paralaxShader.setInt("depthMap", 2);
...
#pragma region MyRegion
        paralaxShader.use();
        paralaxShader.setMat4("projection", projection);
        paralaxShader.setMat4("view", view);
        // render parallax-mapped quad
        model = glm::mat4(1.0f);
        //model = glm::rotate(model, glm::radians((float)glfwGetTime() * -10.0f), glm::normalize(glm::vec3(1.0, 0.0, 1.0))); // rotate the quad to show parallax mapping from multiple directions
        model = glm::translate(model, glm::vec3(0.0f, 0.0f, -4.0f));
        paralaxShader.setMat4("model", model);
        paralaxShader.setVec3("viewPos", camera.position);
        paralaxShader.setVec3("lightPos", lightPos);
        paralaxShader.setFloat("heightScale", heightScale); // adjust with Q and E keys
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, diffuseParalaxMap);
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, normalParalaxMap);
        glActiveTexture(GL_TEXTURE2);
        glBindTexture(GL_TEXTURE_2D, heightParalaxMap);
        renderQuad();
#pragma endregion       
